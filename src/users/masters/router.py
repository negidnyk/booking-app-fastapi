...
async def complete_my_profile(profile: UserUpdate, service_ids: list[int],
                                  session: AsyncSession = Depends(get_async_session),
                                  user: User = Depends(current_active_user)):
    return await complete_masters_profile(profile, session, user)


@router.patch("/me", status_code=201)
async def update_my_profile(profile: UserUpdate, service_ids: list[int],
                                  session: AsyncSession = Depends(get_async_session),
                                  user: User = Depends(current_active_user)):
